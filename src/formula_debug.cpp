...
void
Formula::printFullOccurrenceList(const Variable var, std::ostream& stream) const
{
    assert(var <= maxVarIndex());
    printOccurrenceList(var2lit(var), stream);
    printOccurrenceList(negate(var2lit(var)), stream);
}

void
Formula::printOccurrenceList(const Literal lit, std::ostream& stream) const
{
