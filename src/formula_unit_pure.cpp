...
Formula::pushUnit(const Literal lit, const PureStatus status)
{
    ...
    // Modified: push universal (random) into pure literal stack instead of return UNSAT directly
    if (status == PureStatus::UNIT && isUniversal(var) && !isStochastic()) {
        throw UNSATException("Universal literal is unit.");
    }

    // Modified (added): ignore pure literal for random
    if (status == PureStatus::PURE && isUniversal(var) && isStochastic()) {
        VLOG(3) << "Literal " << lit2dimacs(lit) << " ignored (pure random literal)";
        return;
    }
...
Formula::unitPropagation()
{
    ...
    while (!_unit_stack.empty()) {
        ...
        // Modified: Record probability of eliminated unit random variables
        if (isUniversal(top_var)){
            val_assert(isStochastic());
            val_assert(_prefix->getProb(top_var) >= 0);
            VLOG(2) << "Random unit literal " << top_lit << " eliminated.";
            double prob = isPositive(top_lit) ? _prefix->getProb(top_var) : 1 - _prefix->getProb(top_var);
...
Formula::findPure()
{
    ...
    do {
        ...
        for (Literal lit = minLitIndex(); lit <= maxLitIndex(); ++lit) {
            ...
            // Modified: Skip if current var is random
            if (isStochastic() && isUniversal(lit2var(lit))) {
                continue;
            }
