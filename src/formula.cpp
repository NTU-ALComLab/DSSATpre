...
/**
 * \brief Applies unit propagation, equivalence checking, pure literal detection
...
Formula::updateVars()
{
    ...
    for (Variable var = minVarIndex(); var <= maxVarIndex(); ++var) {
        if (!varDeleted(var) && _occ_list[var2lit(var, false)].empty() && _occ_list[var2lit(var, true)].empty()) {
            // Modified: Added log for variables eliminated by Formula::updateVars()
            VLOG(2) << "Variable " << var << " deleted, since it no longer occurs in the formula.";
            removeVar(var);
        }
    }
...
{
	VLOG(4) << __FUNCTION__ << " for clause nr " << c_nr;
...
{
	VLOG(4) << __FUNCTION__ << " " << lit2dimacs(lit) << " from clause " << c_nr << " (" << _clauses[c_nr] << ")";
    val_assert(!clauseDeleted(c_nr));
...
/**
 * \brief Counts occurrences of all literals and stores it into `_variable_score`
 */
void
...
/**
 * \brief Counts occurrences of all variables and stores it into
 * `_variable_score`
 */
void
Formula::countOccurrencesVar()
{
...
// Modified (Added)
/**
 * @brief If \param val, always treat the formula as (D)SSAT.
 * If the formula is (D)QBF, then the universal varables
 * are treated as random ones with probability 0.5.
 */
void
Formula::enforceStochastic(const bool val) noexcept
{
    _enforce_stochastic = val;
    if (val){
        VLOG(2) << "Enforcing stochastic.";
        setStochastic();
    }
}
...
/**
 * @brief Disable invalid preprocessing techniques when the formula is stochastic.
 * TODO: Re-enable safe ones
 */
void
Formula::setStochastic() noexcept
{
    VLOG(1) << "Formula is stochastic. Disabling invalid processes.";
    _prefix ->setStochastic(true);
    // Known unsound
    _settings.univ_reduction   = false;
    _settings.bce              = 0;
    _settings.hidden           = 0;
    _settings.covered          = false;
    _settings.ble              = false;
    _settings.bla              = false;
    _settings.enableFork       = false;
    // Violates assertion (existing in HQSpre before modification)
    _settings.semantic_gates   = false;
    // Unknown
    _settings.bia              = false;
    _settings.rewrite          = false;
    _settings.extend_gates     = false;
    _settings.univ_expand      = 0;
    // Unknown_high priority
    // TODO: make this default but still able to turn off
    // _settings.hec              = true; // See https://iccl.inf.tu-dresden.de/web/Techreport3036 and https://www.msoos.org/2011/07/note-to-self-higher-level-autarkies/
    // Current Testing
