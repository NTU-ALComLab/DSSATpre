...
main(int argc, char** argv)
{
    ...
    // Modified: Treat universal variables as random ones with probability 0.5
    bool                         enforce_stochastic = false;
    ...
    public_options.add_options()("help,h", "Show available options")(
        ...
        "stochastic", boost::program_options::value<bool>(&enforce_stochastic)->default_value(false),
        "Treat the formula as a (D)SSAT formula (all universal treated as random with probability 0.5)")
#ifdef linux
    ...
    if (verbosity > 0) {
        std::cout << "This is DSSATpre built on " << __DATE__ << " at " << __TIME__ << "\n";
#ifndef NDEBUG
    ...
    formula.enforceStochastic(enforce_stochastic);
    ...
    try {
        ...
        if (!formula.isQBF() && settings.convert_to_qbf == hqspre::DQBFtoQBFmethod::DEP_ELIM) {
            // Modified: Use unit_cost instead in case the exact cost is too big
            // TODO: Cleaner implementation
            std::vector<hqspre::Clause::ClauseData> temp;
            try {
                // const auto elim_set = formula.computeDepElimSet();
                temp = formula.computeDepElimSet();
            } catch (hqspre::ElimSetException& e) {
                VLOG(1) << e.what();
                VLOG(1) << "Recomputing using unit cost.";
                temp = formula.computeDepElimSet(true);
            }
            const auto elim_set(std::move(temp));
        ...
        else if(formula.isQBF() && settings.convert_to_qbf == hqspre::DQBFtoQBFmethod::DEP_ELIM){
            formula.convertToQBF();
        }
        // Modified: change to QBF prefix
        ...
        if (verbosity > 0) {
            ...
            // Modified: Output probability if formula is stochastic
            if (formula.isStochastic()) {
                double prob = (formula.getElimProb() < 0) ? 1 : formula.getElimProb();
                LOG(INFO) << "Satisfying probability: " << std::setprecision(std::numeric_limits<double>::max_digits10) << prob;
            }
            VLOG(1) << e.what();
        }
        if (verbosity == 0 && vm.count("pipe") == 0) {
            ...
            if (formula.isStochastic()) {
                double prob = (formula.getElimProb() < 0) ? 1 : formula.getElimProb();
                std::cout << "Satisfying probability: " << std::setprecision(std::numeric_limits<double>::max_digits10) << prob;
            }
            std::cout << "SAT\n";
        }
        ...
        if (vm.count("pipe") > 0) {
            // Handle probability
            if (formula.isStochastic() && formula.getElimProb() > 0) {
                std::cout << "p cnf 1 1\nr " << std::setprecision(std::numeric_limits<double>::max_digits10) << formula.getElimProb() << " 1 0\n1 0\n";
            }
            else {
                std::cout << sat;
            }
