...
{
    _clause_sizes.resize(_clauses.size(), 0);
...
Formula::removeClauseAndUpdateCandidates(const ClauseID c_nr)
{
    ...
    // Delete clause and its occurrences
    removeClause(c_nr);
        ...
        if (_settings.impl_chains) {
            ...
            if (check_lit != 0) {
                if (_settings.bce > 0) {
                    ...
                    if (_clause_sizes.size() < _clauses.size()) {
                        _clause_sizes.resize(_clauses.size(), 0);
                        _blocked_candidates.resize(_clauses.size());
                    }
                    for (const ClauseID occ_nr : _occ_list[check_lit]) {
                        ...
                        if (!_blocked_candidates.inHeap(occ_nr)) {
                            _clause_sizes[occ_nr] = _clauses[occ_nr].size();
        ...
        for (std::size_t j = 0; j != _occ_list[cur_lit].size(); ++j) {
            ...
            // -> add negated literal to clause
                ...
                if (_seen[negate(candidate)] == 0u) {
                    ...
                    // Unmark occurrence list of added literal
