...
// Modified (Added)
/**
 * \brief Set probability of variable var to prob
 */
void
Prefix::setProb(const Variable var, const double prob)
{
    val_assert(minVarIndex() <= var && var <= maxVarIndex());
    val_assert(isUniversal(var));
    val_assert(isStochastic());
    val_assert(0 <= prob && prob <= 1);
    _probs[var] = prob;
}

// Modified (Added)
/**
 * \brief Get probability of variable var
 */
double
Prefix::getProb(Variable var) const
{
    val_assert(minVarIndex() <= var && var <= maxVarIndex());
    val_assert(isUniversal(var));
    return _probs[var];
 ...
 */
void
DQBFPrefix::write(std::ostream& stream, std::vector<Variable>* const translation_table) const
{
    const auto trans = [translation_table](const Variable var) -> Variable {
        return (translation_table != nullptr ? (*translation_table)[var] : var);
    };

    // Modified: Handle (D)SSAT
    if (isStochastic()) {
        for (Variable var = minVarIndex(); var <= maxVarIndex(); ++var) {
            if (isUniversal(var)) {
                // print random variables
                // TODO (perhaps): group variables of the same probability together
                val_assert(getProb(var) >= 0);
                stream << "r " << std::setprecision(std::numeric_limits<double>::max_digits10) << getProb(var) << " " << trans(var) << " 0\n";
            }
        }
    } else {
 ...
 */
void
QBFPrefix::write(std::ostream& stream, std::vector<Variable>* const translation_table) const
{
    const auto trans = [translation_table](const Variable var) -> Variable {
        return (translation_table != nullptr ? (*translation_table)[var] : var);
    };
    ...
    for (const auto& block : _blocks) {
        ...
        // Modified: Cannot group all randoms in one line due to potentially different probabilities
        if (isUniversal(*(block.begin())) && isStochastic()){
            for (Variable var : block) {
                val_assert(isUniversal(var));
                val_assert(getProb(var) >= 0);
                stream << "r " << std::setprecision(std::numeric_limits<double>::max_digits10) << getProb(var) << " " << trans(var) << " 0\n";
 ...
 */
void
DQBFPrefix::clearDependencies(const Variable var)
{
    val_assert(minVarIndex() <= var && var <= maxVarIndex());
...
DQBFPrefix::convertToQBF() const
{
    ...
    // Modified: Keep stochastic and probs
    result->setStochastic(isStochastic());
    ...
    for (const auto exist : exist_vars) {
        for (const auto dep : _dependencies[exist]) {
            if (!finished_universal[dep]) {
                ...
                result->addUVar(dep);
                // Modified: handle probability
                val_assert(!isStochastic() || getProb(dep) >= 0);
                if (isStochastic()) {
                    result->setProb(dep, getProb(dep));
                }
            }
        }
    ...
    for (Variable univ = minVarIndex(); univ <= maxVarIndex(); ++univ) {
        if (isUniversal(univ) && !finished_universal[univ]) {
            ...
            result->addUVar(univ);
            // Modified: handle probability
            val_assert(!isStochastic() || getProb(univ) >= 0);
            if (isStochastic()) {
                result->setProb(univ, getProb(univ));
            }
        }
    }
    ...
    for (const auto& block : _blocks) {
        for (Variable var : block) {
            ...
            if (isUniversal(var)) {
                result->addUVar(var);
                // Modified: handle probability
                val_assert(!isStochastic() || getProb(var) >= 0);
                if (isStochastic()) {
                    result->setProb(var, getProb(var));
                }
                deps.insert(var);
            } else {
                result->addEVar(var);
...
void
DQBFPrefix::setDependencies(const Variable var, const Container& deps)
{
    val_assert(minVarIndex() <= var && var <= maxVarIndex());
    val_assert(isExistential(var));
    ...
    if (_in_rmb[var]) {
        for (const Variable dep : _univ_vars) {
            _dependencies[dep].erase(var);
        }
        _dependencies[var].clear();
        _in_rmb[var] = false;
...
DQBFPrefix::moveToRMB(const Variable var)
{
    ...
    _dependencies[var].clear();
    _rmb.insert(var);
    for (const Variable univ : _univ_vars) {
